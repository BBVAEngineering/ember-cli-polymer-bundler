...
const getFolder = (filepath) => {
	...
	fragment.childNodes.filter(link).forEach((node) => {
		...
		if (fileExists(path.join(importFolder, 'es6-imports.js'))) {
			let config = require('../config/webpack.config')(importFolder);
...
// 	const litElementParams = Object.assign({}, parameters['lit-element']);
//     const { distributionFolder = 'dist' } = litElementParams;
//     const config = require('../../config/webpack.config');

//     return createLitComponentsHtml(distributionFolder)
//       .then(() => webpack(config))
//       .then(() => moveHtmls(distributionFolder))
//       .then(() => parameters);
//   }
// };
