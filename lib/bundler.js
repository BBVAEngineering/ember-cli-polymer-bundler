...
module.exports = class ElementBundler extends Plugin {
	constructor(inputNode, options, bundlerOptions) {
		...
		this.webpackConfigs = options.webpackConfigs;
	...
	build() {
		...
		return this._processWebpack().then(() => {
	...
	_processWebpack() {
		const config = require('../config/webpack.config')({
			entries: this.webpackConfigs.entries,
			litOutputFolder: this.webpackConfigs.outputFolder,
			litOutputFile: 'lit-imports.html',
			alias: this.webpackConfigs.alias
		});

		return this._createImports(this.webpackConfigs.outputFolder, 'lit-imports.html')
			.then(() => webpack(config));
	}

	_createImports(folder, fileName) {
