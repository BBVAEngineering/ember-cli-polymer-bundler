...
module.exports = class ElementBundler extends Plugin {
	...
	processHTML(input) {
		...
		const jsFileName = path.basename(this.output.replace(path.extname(this.output), '.js'));
	...
	getBabelPressets() {
		const targets = this.buildForProduction.browsers;
	...
	writeFiles(fileContents) {
		...
		const outputPath = path.join(
			this.outputPath,
			this.output.replace(path.extname(this.output), `.${type}`)
		);
		...
		return fs.outputFile(outputPath, content).then(writeRemaining);
	}
};
